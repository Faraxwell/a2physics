...
\sol Firstly consider heat required to heat silver from $293 \mathrm{~K}$ to $1340 \mathrm{~K}$:
$$
...
\question{The variation of its pressure $p$ with its volume $V$ is shown on the graph.} 

\begin{figure}[ht]
	\centering
	\vspace*{-5pt}
	\begin{tikzpicture}[scale=1]
	\draw[<->] (3,8) node[above]{$p$/kPa} -- (3,2) -- (8,2) node[right] {$V/10^{-3}$ m$^3$};
	\draw[help lines, gray] (3,2) grid (8,8);
	\foreach \x in {100,150,200,250,300,350,400} \node[left] at (2.9,\x*2/100) {$\x$};
	\foreach \x in {3,4,5,6,7,8} \node[below] at (\x,1.9) {$\x$};
	\draw[very thick, blue, ->] (4,2.4) node[left]{$A$} -- (4,4.8);
	\draw[very thick, blue] (4,4.8) -- (4,7.2) node[left]{$B$};
	\draw [very thick, blue, domain=4:7.75] plot (\x, {39.68/\x-2.72}) node[right=-0.1]{$C$};
	\draw[very thick, blue, ->] (7.75,2.4) -- (5.7,2.4);
	\draw[very thick, blue] (5.7,2.4) -- (4,2.4);
	\draw[very thick, blue, ->] (6,3.89) -- (6.05,3.84);
	\end{tikzpicture}
	\vspace*{-10pt}
\end{figure}

Giving that the temperature of the gas at $\mathrm{A}$ is $290 \mathrm{~K}$. The temperature at $\mathrm{B}$ is $870 \mathrm{~K}$. The pressure at $\mathrm{A}$ and is $120 \mathrm{~kPa}$. The volume at $\mathrm{C}$ is approximately $7.75\times10^{-3}\mathrm{~m^{-3}}$. Complete the table.
